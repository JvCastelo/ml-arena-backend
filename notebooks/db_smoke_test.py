# %% [markdown]
# # Smoke test do banco (models + db.py)
#
# Roda célula por célula (Shift+Enter no VS Code). Cada célula abre sua
# própria `with get_session()`, simulando uma requisição isolada da API —
# é assim que o Flask vai usar isso, uma sessão por request, nunca uma
# sessão compartilhada entre chamadas.
#
...
with get_session() as session:
    ...
    session.add(user)
    session.flush()  # atribui o user.id sem esperar o fim do "with"
    user_id = user.id
...
with get_session() as session:
    print("status continua:", session.get(Run, run_id).status)
...
# %%
with get_session() as session:
    before = session.scalars(select(Artifact).where(Artifact.run_id == run_id)).all()
    print("artifacts antes do delete:", len(before))

    session.delete(session.get(Run, run_id))

with get_session() as session:
    after = session.scalars(select(Artifact).where(Artifact.run_id == run_id)).all()
...
with get_session() as session:
    print("user ainda existe?", session.get(User, user_id) is not None)
